...
try:
    ...
    print(f"✅ RealCrossChainBridge importado com sucesso!")
except ImportError as import_error:
    REAL_BRIDGE_AVAILABLE = False
    RealCrossChainBridge = None
    print(f"⚠️  Erro ao importar RealCrossChainBridge: {import_error}")
    ...
    def real_transfer(
        ...
        if source_chain.lower() in ['allianza', 'alz']:
            ...
            if not self.real_bridge:
                # Tentar inicializar bridge apenas para destino
                print(f"⚠️  Real bridge não disponível, tentando inicializar apenas para destino...")
                print(f"🔍 REAL_BRIDGE_AVAILABLE: {REAL_BRIDGE_AVAILABLE}")
                print(f"🔍 RealCrossChainBridge: {RealCrossChainBridge}")
                try:
                    if REAL_BRIDGE_AVAILABLE and RealCrossChainBridge:
                        print(f"🔍 Tentando inicializar RealCrossChainBridge...")
                        self.real_bridge = RealCrossChainBridge()
                        print(f"✅ Bridge inicializado para transferência ALZ → {target_chain}")
                        # Se conseguiu inicializar, continuar com transferência real
                        if self.real_bridge:
                            print(f"✅ Bridge disponível! Continuando com transferência REAL...")
                            # Não retornar aqui, deixar continuar para usar o bridge
                    else:
                        # Se não conseguir inicializar bridge, fazer transferência simulada mas registrar
                        print(f"⚠️  Bridge não disponível (REAL_BRIDGE_AVAILABLE={REAL_BRIDGE_AVAILABLE}, RealCrossChainBridge={RealCrossChainBridge})")
                        print(f"⚠️  Criando transferência simulada com provas ALZ-NIEV")
                        result = self._create_alz_to_external_transfer(
                            target_chain=target_chain,
                            amount=amount,
                            recipient=recipient,
                        ...
                        print(f"✅ Transferência simulada criada: {result.get('success')}")
                        return result
                except Exception as bridge_init_error:
                    import traceback
                    print(f"⚠️  Erro ao inicializar bridge: {bridge_init_error}")
                    traceback.print_exc()
                    # Tentar usar bridge mesmo com erro (pode funcionar parcialmente)
                    if self.real_bridge:
                        print(f"⚠️  Bridge inicializado apesar do erro, tentando usar mesmo assim...")
                        # Continuar para tentar usar o bridge
                    else:
                        # Criar transferência simulada com provas ALZ-NIEV
                        print(f"⚠️  Criando transferência simulada após erro na inicialização")
                        result = self._create_alz_to_external_transfer(
                            target_chain=target_chain,
                            amount=amount,
                            recipient=recipient,
                        ...
                        print(f"✅ Transferência simulada criada após erro: {result.get('success')}")
                        return result
            
            # Se chegou aqui e ainda não tem bridge após tentar inicializar, criar transferência simulada
            if not self.real_bridge:
                print(f"⚠️  Bridge ainda não disponível após tentativas, criando transferência simulada")
                result = self._create_alz_to_external_transfer(
                    target_chain=target_chain,
                    amount=amount,
                    recipient=recipient,
                ...
                print(f"✅ Transferência simulada criada (fallback): {result.get('success')}")
                return result
            
            # Se chegou aqui, o bridge está disponível! Continuar com transferência REAL
            print(f"✅ Bridge disponível! Executando transferência REAL ALZ → {target_chain}")
